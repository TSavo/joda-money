 ...
 * <p>
 * This class represents a quantity of money, stored as a {@code BigDecimal}
 * amount in a single {@link CurrencyUnit currency}.
 * <p>
 * Every currency has a certain standard number of decimal places. This is
 * typically 2 (Euro, British Pound, US Dollar) but might be 0 (Japanese Yen), 1
 * (Vietnamese Dong) or 3 (Bahrain Dinar). The {@code BigMoney} class is not
 * restricted to the standard decimal places and can represent an amount to any
 * precision that a {@code BigDecimal} can represent.
 * <p>
...
public final class BigMoney implements BigMoneyProvider, Comparable<BigMoneyProvider>, Serializable {
	 ...
	 * the most expected answer for most programming scenarios. Any {@code double}
	 * literal in code will be converted to exactly the same BigDecimal with the
	 * same scale. For example, the literal '1.425d' will be converted to '1.425'.
	...
	// -----------------------------------------------------------------------
	/**
	 * Obtains an instance of {@code BigMoney} from a {@code BigDecimal} at a
	 * specific scale.
	 * <p>
	 * This allows you to create an instance with a specific currency and amount.
	 * No rounding is performed on the amount, so it must have a scale less than
	 * or equal to the new scale.
	 * 
	 * @param currency
	 *          the currency, not null
	 * @param amount
	 ...
	 * <p>
	 * This allows you to create an instance with a specific currency and amount.
	 * If the amount has a scale in excess of the scale of the currency then the
	 * excess fractional digits are rounded using the rounding mode.
	 * 
	 * @param currency
	 *          the currency, not null
	 * @param amount
	 ...
	 * <p>
	 * This allows you to create an instance with a specific currency, amount and
	 * scale. The amount is defined in terms of the specified scale.
	 * <p>
	 * For example, {@code ofScale(USD, 234, 2)} creates the instance
	 * {@code USD 2.34}.
	 * 
	 * @param currency
	 *          the currency, not null
	 * @param unscaledAmount
	 *          the unscaled amount of money
	 * @param scale
	 *          the scale to use
	 * @return the new instance, never null
	 */
	 ...
	 * <p>
	 * The amount is a whole number only. Thus you can initialise the value 'USD
	 * 20', but not the value 'USD 20.32'. For example, {@code ofMajor(USD, 25)}
	 * creates the instance {@code USD 25}.
	 * 
	 * @param currency
	 *          the currency, not null
	 * @param amountMajor
	 ...
	 * <p>
	 * This allows you to create an instance with a specific currency and amount
	 * expressed in terms of the minor unit. The scale of the money will be that
	 * of the currency, such as 2 for USD or 0 for JPY.
	 * <p>
	 * For example, if constructing US Dollars, the input to this method
	 * represents cents. Note that when a currency has zero decimal places, the
	 * major and minor units are the same. For example, {@code ofMajor(USD, 2595)}
	 * creates the instance {@code USD 25.95}.
	 * 
	 * @param currency
	 *          the currency, not null
	 * @param amountMinor
	 ...
	 * <p>
	 * The scale of the money will be zero. For example, {@code zero(USD)} creates
	 * the instance {@code USD 0}.
	 * 
	 * @param currency
	 *          the currency, not null
	...
	/**
	 * Obtains an instance of {@code BigMoney} representing zero at a specific
	 * scale.
	 * <p>
	 ...
	 * provider, such as {@code Money}. This method simply calls
	 * {@link BigMoneyProvider#toBigMoney()} checking for nulls.
	 * 
	 * @param moneyProvider
	 ...
	 * <p>
	 * The array must contain at least one monetary value. Subsequent amounts are
	 * added as though using {@link #plus(BigMoneyProvider)}. All amounts must be
	 ...
	 * are added as though using {@link #plus(BigMoneyProvider)}. All amounts must
	...
	/**
	 * Obtains an instance of {@code Money} as the total value of a possibly empty
	 * array.
	 * <p>
	 * The amounts are added as though using {@link #plus(BigMoneyProvider)}
	 * starting from zero in the specified currency. All amounts must be in the
	 * same currency.
	 * 
	 * @param currency
	 ...
	 * <p>
	 * The string format is '<currencyCode> <amount>'. The currency code must be a
	 * valid three letter currency. The amount must match the regular expression
	 * {@code [+-]?[0-9]*[.]?[0-9]*}. This matches the output from
	 * {@link #toString()}.
	 * <p>
	...
	public static BigMoney parse(String moneyStr) {
		...
		if (moneyStr.length() < 5 || moneyStr.charAt(3) != ' ' && moneyStr.charAt(4) != ' ') {
		...
		if (moneyStr.charAt(3) == ' ') {
			currStr = moneyStr.substring(0, 3);
			amountStr = moneyStr.substring(4);
		} else {
			currStr = moneyStr.substring(0, 4);
			amountStr = moneyStr.substring(5);
	 ...
	 * <p>
	 * If the input money is not {@code null}, then it is returned, providing that
	 * the currency matches the specified currency. If the input money is
	 * {@code null}, then zero money in the currency is returned with a scale of
	 * zero.
	 * 
	 * @param money
	 ...
	 * <p>
	 * The returned instance will have the specified currency and the amount from
	 * this instance. No currency conversion or alteration to the scale occurs.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 * @param currency
	 ...
	 * <p>
	 * The scale has the same meaning as in {@link BigDecimal}. Positive values
	 * represent the number of decimal places in use. Negative numbers represent
	 * the opposite. For example, a scale of 2 means that the money will have two
	 * decimal places such as 'USD 43.25'. Whereas, a scale of -3 means that only
	 * thousands can be represented, such as 'GBP 124000'.
	 * 
	 ...
	 * <p>
	 * Each currency has a default scale, such as 2 for USD and 0 for JPY. This
	 * method checks if the current scale matches the default scale.
	 * 
	...
	// -----------------------------------------------------------------------
	/**
	 * Returns a copy of this monetary value with the specified scale, truncating
	 * the amount if necessary.
	 * <p>
	 * The returned instance will have this currency and the new scaled amount.
	 * For example, scaling 'USD 43.271' to a scale of 1 will yield 'USD 43.2'. No
	 * rounding is performed on the amount, so it must have a scale less than or
	 * equal to the new scale.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 * @param scale
	...
	/**
	 * Returns a copy of this monetary value with the specified scale, using the
	 * specified rounding mode if necessary.
	 * <p>
	 * The returned instance will have this currency and the new scaled amount.
	 ...
	 * <p>
	 * The returned instance will have this currency and the new scaled amount.
	 * For example, scaling 'USD 43.271' will yield 'USD 43.27' as USD has a scale
	 * of 2. No rounding is performed on the amount, so it must have a scale less
	 * than or equal to the new scale.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 * @return the new instance with the input amount set, never null
	...
	/**
	 * Returns a copy of this monetary value with the scale of the currency, using
	 * the specified rounding mode if necessary.
	 * <p>
	 * The returned instance will have this currency and the new scaled amount.
	 * For example, scaling 'USD 43.271' will yield 'USD 43.27' as USD has a scale
	 * of 2.
	 ...
	 * Gets the amount.
	 * <p>
	 * This returns the value of the money as a {@code BigDecimal}. The scale will
	 * be the scale of this money.
	 * 
	 ...
	 * currency, truncating the amount if necessary. For example, 'EUR 2.35' will
	 * return 2, and 'BHD -1.345' will return -1.
	 * <p>
	 * This is returned as a {@code BigDecimal} rather than a {@code BigInteger}.
	 * This is to allow further calculations to be performed on the result. Should
	 * you need a {@code BigInteger}, simply call
	 ...
	 * currency, truncating the amount if necessary. For example, 'EUR 2.35' will
	 * return 235, and 'BHD -1.345' will return -1345.
	 * <p>
	 * This is returned as a {@code BigDecimal} rather than a {@code BigInteger}.
	 * This is to allow further calculations to be performed on the result. Should
	 * you need a {@code BigInteger}, simply call
	 ...
	 * <p>
	 * This return the minor unit part of the monetary amount. This is defined as
	 * the amount in minor units excluding major units.
	 * <p>
	 * For example, EUR has a scale of 2, so the minor part is always between 0
	 * and 99 for positive amounts, and 0 and -99 for negative amounts. Thus 'EUR
	 * 2.35' will return 35, and 'EUR -1.34' will return -34.
	 * 
	 * @return the minor part of the amount, negative if the amount is negative
	 */
	public int getMinorPart() {
		int cdp = getCurrencyUnit().getDecimalPlaces();
		return amount.setScale(cdp, RoundingMode.DOWN).remainder(BigDecimal.ONE).movePointRight(cdp).intValueExact();
	}
	 ...
	 * <p>
	 * The returned instance will have this currency and the new amount. The scale
	 * of the returned instance will be that of the specified BigDecimal.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 * @param amount
	...
	/**
	 * Returns a copy of this monetary value with the specified amount using a
	 * well-defined conversion from a {@code double}.
	 * <p>
	...
	// -----------------------------------------------------------------------
	/**
	 * Returns a copy of this monetary value with a collection of monetary amounts
	 * added.
	 * <p>
	 * This adds the specified amounts to this monetary amount, returning a new
	 * object. The amounts are added as though using
	 * {@link #plus(BigMoneyProvider)}. The amounts must be in the same currency.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 * @param moniesToAdd
	 ...
	 * <p>
	 * This adds the specified amount to this monetary amount, returning a new
	 * object. The amount added must be in the same currency.
	 * <p>
	 * No precision is lost in the result. The scale of the result will be the
	 * maximum of the two scales. For example, 'USD 25.95' plus 'USD 3.021' gives
	 * 'USD 28.971'.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 ...
	 * <p>
	 * No precision is lost in the result. The scale of the result will be the
	 * maximum of the two scales. For example, 'USD 25.95' plus '3.021' gives 'USD
	 * 28.971'.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 ...
	 * <p>
	 * No precision is lost in the result. The scale of the result will be the
	 * maximum of the two scales. For example, 'USD 25.95' plus '3.021d' gives
	 * 'USD 28.971'.
	 * <p>
	 * The amount is converted via {@link BigDecimal#valueOf(double)} which yields
	 * the most expected answer for most programming scenarios. Any {@code double}
	 * literal in code will be converted to exactly the same BigDecimal with the
	 * same scale. For example, the literal '1.45d' will be converted to '1.45'.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 * @param amountToAdd
	 ...
	 * <p>
	 * No precision is lost in the result. The scale of the result will be the
	 * maximum of the current scale and 0. For example, 'USD 23.45' plus '138'
	 * gives 'USD 161.45'.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 ...
	 * <p>
	 * No precision is lost in the result. The scale of the result will be the
	 * maximum of the current scale and the default currency scale. For example,
	 * 'USD 23.45' plus '138' gives 'USD 24.83'.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 * @param amountToAdd
	...
	// -----------------------------------------------------------------------
	/**
	 * Returns a copy of this monetary value with the amount in the same currency
	 * added retaining the scale by rounding the result.
	 * <p>
	 * The scale of the result will be the same as the scale of this instance. For
	 * example,'USD 25.95' plus 'USD 3.021' gives 'USD 28.97' with most rounding
	 * modes.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	...
	/**
	 * Returns a copy of this monetary value with the amount added retaining the
	 * scale by rounding the result.
	 * <p>
	 * The scale of the result will be the same as the scale of this instance. For
	 * example,'USD 25.95' plus '3.021' gives 'USD 28.97' with most rounding
	 * modes.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 ...
	 * <p>
	 * The scale of the result will be the same as the scale of this instance. For
	 * example,'USD 25.95' plus '3.021d' gives 'USD 28.97' with most rounding
	 * modes.
	 * <p>
	 * The amount is converted via {@link BigDecimal#valueOf(double)} which yields
	 * the most expected answer for most programming scenarios. Any {@code double}
	 ...
	 * subtracted.
	 * <p>
	 * This subtracts the specified amounts from this monetary amount, returning a
	 * new object. The amounts are subtracted one by one as though using
	 * {@link #minus(BigMoneyProvider)}. The amounts must be in the same currency.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 * @param moniesToSubtract
	 ...
	 * <p>
	 * This subtracts the specified amount from this monetary amount, returning a
	 * new object. The amount subtracted must be in the same currency.
	 * <p>
	 * No precision is lost in the result. The scale of the result will be the
	 * maximum of the two scales. For example,'USD 25.95' minus 'USD 3.021' gives
	 * 'USD 22.929'.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 ...
	 * <p>
	 * No precision is lost in the result. The scale of the result will be the
	 * maximum of the two scales. For example,'USD 25.95' minus '3.021' gives 'USD
	 * 22.929'.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 ...
	 * <p>
	 * No precision is lost in the result. The scale of the result will be the
	 * maximum of the two scales. For example,'USD 25.95' minus '3.021d' gives
	 * 'USD 22.929'.
	 * <p>
	 * The amount is converted via {@link BigDecimal#valueOf(double)} which yields
	 * the most expected answer for most programming scenarios. Any {@code double}
	...
	/**
	 * Returns a copy of this monetary value with the amount in major units
	 * subtracted.
	 * <p>
	 * This subtracts the specified amount in major units from this monetary
	 * amount, returning a new object. The minor units will be untouched in the
	 * result.
	 * <p>
	 * No precision is lost in the result. The scale of the result will be the
	 * maximum of the current scale and 0. For example, 'USD 23.45' minus '138'
	 * gives 'USD -114.55'.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	...
	/**
	 * Returns a copy of this monetary value with the amount in minor units
	 * subtracted.
	 * <p>
	 * This subtracts the specified amount in minor units from this monetary
	 * amount, returning a new object.
	 * <p>
	 * No precision is lost in the result. The scale of the result will be the
	 * maximum of the current scale and the default currency scale. For example,
	 * USD 23.45 minus '138' gives 'USD 22.07'.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 * @param amountToSubtract
	...
	/**
	 * Returns a copy of this monetary value with the amount in the same currency
	 * subtracted retaining the scale by rounding the result.
	 * <p>
	 * The scale of the result will be the same as the scale of this instance. For
	 * example,'USD 25.95' minus 'USD 3.029' gives 'USD 22.92 with most rounding
	 * modes.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 ...
	 * <p>
	 * The scale of the result will be the same as the scale of this instance. For
	 * example,'USD 25.95' minus '3.029' gives 'USD 22.92' with most rounding
	 * modes.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 ...
	 * <p>
	 * The scale of the result will be the same as the scale of this instance. For
	 * example,'USD 25.95' minus '3.029d' gives 'USD 22.92' with most rounding
	 * modes.
	 * <p>
	 * The amount is converted via {@link BigDecimal#valueOf(double)} which yields
	 * the most expected answer for most programming scenarios. Any {@code double}
	 ...
	 * <p>
	 * No precision is lost in the result. The result has a scale equal to the sum
	 * of the two scales. For example, 'USD 1.13' multiplied by '2.5' gives 'USD
	 * 2.825'.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 ...
	 * scale of this money. For example, 'USD 1.13' multiplied by '2' gives 'USD
	 * 2.26'.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 ...
	 * <p>
	 * This multiplies this money by the specified value, retaining the scale of
	 * this money. This will frequently lose precision, hence the need for a
	 * rounding mode. For example, 'USD 1.13' multiplied by '2.5' and rounding
	 * down gives 'USD 2.82'.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 ...
	 *          the scalar value to multiply by, not null
	 * @param roundingMode
	 *          the rounding mode to use to bring the decimal places back in line,
	 *          not null
	 * @return the new multiplied instance, never null
	 * @throws ArithmeticException
	 *           if the rounding fails
	...
	// -----------------------------------------------------------------------
	/**
	 * Returns a copy of this monetary value divided by the specified value using
	 * the specified rounding mode to adjust the scale.
	 * <p>
	 * The result has the same scale as this instance. For example, 'USD 1.13'
	 * divided by '2.5' and rounding down gives 'USD 0.45' (amount rounded down
	 * from 0.452).
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 * @param valueToDivideBy
	 *          the scalar value to divide by, not null
	 * @param roundingMode
	 *          the rounding mode to use, not null
	...
	/**
	 * Returns a copy of this monetary value divided by the specified value using
	 * the specified rounding mode to adjust the decimal places in the result.
	 * <p>
	 * The result has the same scale as this instance. For example, 'USD 1.13'
	 * divided by '2' and rounding down gives 'USD 0.56' (amount rounded down from
	 * 0.565).
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 ...
	 * This instance is immutable and unaffected by this method.
	 * 
	 * @return the new instance with the amount converted to be positive, never
	 *         null
	 */
	public BigMoney abs() {
	 ...
	 * without changing the current scale.
	 * <p>
	 * Scale is described in {@link BigDecimal} and represents the point below
	 * which the monetary value is zero. Negative scales round increasingly large
	 * numbers. Unlike {@link #withScale(int)}, this scale of the result is
	 * unchanged.
	 * <ul>
	 * <li>Rounding 'EUR 45.23' to a scale of -1 returns 40.00 or 50.00 depending
	 * on the rounding mode.
	 * <li>Rounding 'EUR 45.23' to a scale of 0 returns 45.00 or 46.00 depending
	 * on the rounding mode.
	 * <li>Rounding 'EUR 45.23' to a scale of 1 returns 45.20 or 45.30 depending
	 * on the rounding mode.
	 * <li>Rounding 'EUR 45.23' to a scale of 2 has no effect (it already has that
	 * scale).
	 * <li>Rounding 'EUR 45.23' to a scale of 3 has no effect (the scale is not
	 * increased).
	 * </ul>
	 * This instance is immutable and unaffected by this method.
	 * 
	...
	// -----------------------------------------------------------------------
	/**
	 * Returns a copy of this monetary value converted into another currency using
	 * the specified conversion rate.
	 * <p>
	 * The scale of the result will be the sum of the scale of this money and the
	 * scale of the multiplier. If desired, the scale of the result can be
	 * adjusted to the scale of the new currency using
	 * {@link #withCurrencyScale()}.
	 * <p>
	 * This instance is immutable and unaffected by this method.
	 * 
	 * @param currency
	 *          the new currency, not null
	 * @param conversionMultipler
	...
	/**
	 * Returns a copy of this monetary value converted into another currency using
	 * the specified conversion rate, with a rounding mode used to adjust the
	 * decimal places in the result.
	 * <p>
	 * The result will have the same scale as this instance even though it will be
	...
	// -----------------------------------------------------------------------
	/**
	 * Implements the {@code BigMoneyProvider} interface, trivially returning
	 * {@code this}.
	 * 
	...
	/**
	 * Converts this money to an instance of {@code Money} without rounding. If
	 * the scale of this money exceeds the currency scale an exception will be
	 * thrown.
	 * 
	 * @return the money instance, never null
	 * @throws ArithmeticException
	...
	// -----------------------------------------------------------------------
	/**
	 * Compares this monetary value to another. The compared values must be in the
	 * same currency.
	 * 
	 * @param other
	 ...
	 * <p>
	 * This ignores the scale of the amount. Thus, 'USD 30.00' and 'USD 30' are
	 * equal.
	 * <p>
	...
	/**
	 * Checks if this monetary value is greater than another. The compared values
	...
	/**
	 * Checks if this monetary value is less than another. The compared values
	 * must be in the same currency.
	 * 
	 * @param other
	 *          the other monetary value, not null
	 ...
	 * <p>
	 * Like BigDecimal, this method compares the scale of the amount. Thus, 'USD
	 * 30.00' and 'USD 30' are not equal.
	 * <p>
	 * The compared values must be in the same currency.
	 * 
	 * @param other
	...
	public boolean equals(Object other) {
		...
		if (other instanceof BigMoney) {
			...
			return currency.equals(otherMoney.getCurrencyUnit()) && amount.equals(otherMoney.amount);
		}
		return false;
	}
	 ...
	 * <p>
	 * The format is the 3 letter ISO currency code, followed by a space, followed
	 * by the amount as per {@link BigDecimal#toPlainString()}.
	 * 
	...
	public String toString() {
		return new StringBuilder().append(currency.getCode()).append(' ').append(amount.toPlainString()).toString();
	}
